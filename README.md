...
The browser app reads the config list, config bodies, and per-file history through the Azure Functions promote API, which in turn reads the live repository state from GitHub as the signed-in user. Users must sign in with GitHub before any config data loads, and merged config changes still show up in the UI without waiting for generated browser data to be refreshed. The GitHub Pages workflow now only republishes the browser app itself when its files change.

The details page can also promote a config to the next environment. The published browser app signs the user in with a GitHub App via the Azure Functions backend in `src/config-promote-api`; that same signed-in session is used for both live reads and promotion, and the backend creates a branch, copies the config into the target `configs/<tenant>/<environment>/...` path, and opens a pull request as the signed-in user.
